...
- Existing approved videos: open Admin > Media and select Create video preview for each video. This uses the existing authenticated storage permissions. Videos without a real poster remain playable, use the brand share card and do not emit incomplete VideoObject markup. Poster availability refreshes within five minutes.
